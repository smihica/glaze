...
reader_t::reader_t(int fd)
{
	init();
    m_fd = fd;
}

reader_t::reader_t(FILE* fp)
{
	init();
	m_fp = fp;
}

reader_t::reader_t(const char* src)
{
	init();
	m_src = src;
}
...
inline obj_t* reader_t::make_list_2items(obj_t* first, obj_t* second) {
	return new cons_t(first, (new cons_t(second, g_obj_nil)));
}
...
obj_t* reader_t::read_list(bool bracketed)
{
    ...
    while ((token = read_token()) != S_EOF) {
        if (token == S_RPAREN) {
            ...
            lst = reverse_list(lst, g_obj_nil);
            return lst;
        }
        if (token == S_RBRACK) {
            if (!bracketed) {
...
{
	return const_cast<symbol_t*>(g_symbol_table.get(buf));
}
